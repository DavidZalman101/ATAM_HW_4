	...
	else if( result == STB_GLOBAL )
	{
		...
		else
		{
			*error_val = SYM_FOUND_GLOBAL_NOT_DEFINED_HERE;
			...
			//res_ = find_sym_got_address(fd, &file_header_ptr);
		}
	}
						   ...
						   char* symbol_name, int* symbol_index)
{
	...
	for (int i = 0; i < number_of_entries; i++)
	{
		...
		char* string_to_read = find_string_in_strtab(fd, strtab_section_header, &entire);
		if (string_to_read == NULL )
			continue;
								   ...
								   Elf64_Shdr* strtab_section_header, char* symbol_name)
{
	...
	if (find_dynstr_section(fd, file_header_ptr, shstrtab_section_header, &dynsym_section_header) == false)
	{
		fclose(fd);
		return 0;
	}
	...
	if (find_the_symbol_index(fd, &dynsym_section_header, strtab_section_header, symbol_name, &symbol_index) == false)
	{
		fclose(fd);
		return 0;
	}
